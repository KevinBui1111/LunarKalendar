...
function getMonth2(mm, yy) {
  ...
  for (let i = 0; i < 42; ++i) {
    result.push(getLunarDate(day.getDate(), day.getMonth() + 1, day.getFullYear()));
...
function printTable(mm, yy) {
  ...
  var currentMonth = getMonth2(mm, yy);
  ...
  solar_day.setDate(solar_day.getDate() -
    (get_day_week_nth(solar_day) == 1 ? 7 : get_day_week_nth(solar_day) - 1)
  );
  for (i = 0; i < 6; i++) {
    res += ("<tr>\n");
    for (j = 0; j < 7; j++) {
      ...
      //}
      ld1 = currentMonth[k];
      res += printCell(ld1, solar_day.getDate(), solar_day.getMonth() + 1, solar_day.getFullYear());
      solar_day.setDate(solar_day.getDate() + 1);
    }
    res += ("</tr>\n");
  }
  res += ('</table>\n');
